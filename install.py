#!/usr/bin/env python3
"""
Cross-platform installation script for Terrascope QGIS Plugin.
...
def install_plugin(
    source_dir: Path, plugin_dir: Path, plugin_name: str = "terrascope"
) -> bool:
...
def remove_plugin(plugin_dir: Path, plugin_name: str = "terrascope") -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Install or remove Terrascope Plugin")
    parser.add_argument(
        "--remove",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="terrascope",
        help="Plugin folder name in QGIS plugins directory (default: terrascope)",
    )
    args = parser.parse_args()

    # Get script directory
    script_dir = Path(__file__).parent.resolve()
    source_dir = script_dir / "terrascope"
