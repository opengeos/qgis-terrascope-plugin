#!/usr/bin/env python3
"""
Package the Terrascope QGIS plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder renamed from 'terrascope' to the target plugin name
...
Examples:
    ...
    python package_plugin.py --name terrascope
"""
...
def package_plugin(
    ...
    target_name: str = "terrascope",
    include_version: bool = True,
) -> Path:
    """Package the QGIS plugin into a zip file.

    Args:
        source_dir: Path to the terrascope directory.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Package Terrascope QGIS plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Source directory (default: terrascope in the same folder as this script)",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        default="terrascope",
        help="Target plugin name for the zip root folder (default: terrascope)",
    )
    parser.add_argument(
        "--no-version",
        action="store_true",
    ...
    source_dir = args.source if args.source else script_dir / "terrascope"
